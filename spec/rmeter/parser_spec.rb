  ...
  describe "#parse" do
    it "should return an array of hashes with the parsed values" do
      ...
      <testResults version="1.2">
        <httpSample t="780" lt="746" ts="1309493042836" s="true" lb="Home Page" rc="200" rm="OK" tn="First Group 1-1" dt="text" b="9861"/>
      </testResults>
      XML

      File.should_receive(:open).with('/some/file/path.xml').and_return(StringIO.new(xml))
      Rmeter::Parser.new('/some/file/path.xml').parse.should == [
        {
          :time => 780,
          :latency => 746,
